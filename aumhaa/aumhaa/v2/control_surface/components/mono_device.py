	...
	def _assign_parameters(self, host, *a):
		...
		if(host.is_enabled()):
			...
			if (self._device.class_name in list(self._device_banks.keys())):
				...
				class_name = 'Other'
			if (class_name in list(self._device_banks.keys())): #modified
				assert (class_name in list(self._device_best_banks.keys()))
				banks = self._device_banks[class_name]
				if '_alt_device_banks' in dir(host):
					if self._type in list(host._alt_device_banks.keys()):
						if (class_name in list(host._alt_device_banks[self._type].keys())):
				...
				if (len(banks) > self._bank_index):
					...
					if self._is_banking_enabled(): #added
						if class_name in list(self._device_bank_names.keys()): #added
