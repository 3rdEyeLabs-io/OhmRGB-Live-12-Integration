...
import os, builtins, __main__, _ast, _codecs, _functools, _md5, _random, _sha, _sha256, _sha512, _socket, _sre, _ssl, _struct, _symtable, _weakref, binascii, io, collections, datetime, errno, exceptions, gc, imp, itertools, marshal, math, sys, time
import importlib
...
class Debug(ControlSurface):
	...
	def _scan(self):
		...
		modules = [m.__file__ for m in list(sys.modules.values())
	...
	def rebuild_sys(self):
		...
		modules = [m for m in list(sys.modules.values()) if _normalize_filename(getattr(m, '__file__', None)) in filenames]
